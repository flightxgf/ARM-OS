...
def   wr( fd, req ) :
  ...
  if( address     >= args.block_num ) :
    logging.info( 'Failed to write to %d because address was out of range' % ( address) )
    return [ ACK_FAIL ]
  if( len( data ) != args.block_len ) :
    logging.info( 'Failed to write to %d because data was wrong size (%d)' % ( address, len( data )) )
    return [ ACK_FAIL ]
  ...
  if( len( data ) != n              ) :
    logging.info( 'Failed to write to 0x%X because data was wrong size (%d)' % ( address, len( data )) )
    return [ ACK_FAIL ]

  os.fsync( fd )

  logging.info( 'wr %d bytes -> address 0x%X' % ( len( data ), address) )
...
def   rd( fd, req ) :
  ...
  if( address     >= args.block_num ) :
    logging.info( 'Failed to read byte %d because address was out of range' % ( address) )
    return [ ACK_FAIL ]
