...
int disk_get_block_num() {
  ...
  for( int i = 0; i < DISK_RETRY; i++ ) {
      PL011_puth( UART1, 0x00, true );        // write command
      PL011_putc( UART1, '\n', true );        // write EOL

    if( PL011_geth( UART1, true ) == 0x00 ) { // read  command
      PL011_getc( UART1,       true );        // read  separator
       data_geth( UART1, x, n, true );        // read  data
      PL011_getc( UART1,       true );        // read  EOL
...
int disk_wr( uint32_t a, const uint8_t* x, int n ) {
  for( int i = 0; i < DISK_RETRY; i++ ) {
      PL011_puth( UART1, 0x01, true );        // write command
      PL011_putc( UART1, ' ',  true );        // write separator
       addr_puth( UART1, a,    true );        // write address
      PL011_putc( UART1, ' ',  true );        // write separator
       data_puth( UART1, x, n, true );        // write data
      PL011_putc( UART1, '\n', true );        // write EOL
...
int disk_rd( uint32_t a,       uint8_t* x, int n ) {
  for( int i = 0; i < DISK_RETRY; i++ ) {
      PL011_puth( UART1, 0x02, true );        // write command
      PL011_putc( UART1, ' ',  true );        // write separator
       addr_puth( UART1, a,    true );        // write address
      PL011_putc( UART1, '\n', true );        // write EOL
